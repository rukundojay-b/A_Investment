...
// @route POST /api/user/withdraw - FIXED with 2,500 minimum
router.post('/withdraw', authMiddleware, async (req, res) => {
  try {
    const { amount, paymentMethod, phoneNumber, description } = req.body;
    ...
    // FIXED: Changed from 5000 to 2500
    if (amount < 2500) {
      return res.status(400).json({ 
        success: false, 
        message: 'Minimum withdrawal is 2,500 FRW' 
      });
    }
    ...
    console.log("💰 Bonus status:", {
      ...
      canWithdraw: user.bonus?.canWithdraw,
      investedToActivate: user.bonus?.investedToActivate
    });
    ...
    // This will trigger the bonus lock check in the User model
    const transaction = await user.createWithdrawalRequest(
      amount,
      paymentMethod,
      phoneNumber,
    ...
    // Check if it's the bonus lock error
    if (error.message.includes('You must make an investment first')) {
      return res.status(400).json({ 
        success: false, 
    ...
    if (isFirstInvestment && user.referredBy) {
      try {
        ...
        if (referrer) {
          ...
          await referrer.addNotification(
            `🎉 You earned ${commission.toLocaleString()} FRW commission from ${user.izina_ryogukoresha}'s first investment!`,
            'success'
          );
