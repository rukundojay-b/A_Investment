...
//   wallets: {
//     main: { type: Number, default: 0 }, // Changed from 400 to 0
//     earning: { type: Number, default: 2500 }, // Default bonus
//     reserved: { type: Number, default: 0 }
//   },
...
//     isClaimed: { type: Boolean, default: true }, // Auto-claimed on signup
//     canWithdraw: { type: Boolean, default: false }, // Locked until investment
//     investedToActivate: { type: Boolean, default: false },
//     createdAt: { type: Date, default: Date.now },
...
//       const currentWallets = this.wallets;
//       this.wallets = {
//         main: currentWallets.main !== undefined ? currentWallets.main : 0,
//         earning: currentWallets.earning !== undefined ? currentWallets.earning : 2500,
//         reserved: currentWallets.reserved !== undefined ? currentWallets.reserved : 0
//       };
//     }
...
//   const bonusStatus = {
//     amount: this.bonus?.amount || 2500,
//     isClaimed: this.bonus?.isClaimed || false,
//     canWithdraw: this.bonus?.canWithdraw || false,
//     investedToActivate: this.bonus?.investedToActivate || false,
//     activatedAt: this.bonus?.activatedAt || null,
//     message: !this.bonus?.canWithdraw 
//       ? 'Make your first investment to unlock your 2,500 FRW bonus!' 
//       : 'Your 2,500 FRW bonus is ready for withdrawal!'
//   };
...
//     timezone: this.timezone || 'UTC',
//     lastPaymentDate: this.lastPaymentDate,
//     bonus: bonusStatus, // Add bonus info
//     wallets: {
//       main: this.wallets?.main || 0,
//       earning: this.wallets?.earning || 0,
//       reserved: this.wallets?.reserved || 0,
...
// // ==================================================
// // UPDATED: Create withdrawal request with bonus logic
// // ==================================================
// userSchema.methods.createWithdrawalRequest = async function (amount, paymentMethod, phoneNumber, description) {
//   try {
//     console.log("🔍 createWithdrawalRequest - START");
...
//     // Minimum withdrawal is 2,500 FRW
//     if (amount < 2500) throw new Error('Minimum withdrawal is 2,500 FRW');

//     // Calculate fees (15% service fee + 5% tax)
//     const serviceFee = amount * 0.15;
//     const tax = amount * 0.05;
//     const totalFees = serviceFee + tax;
//     const netAmount = amount - totalFees;

//     // Check if user is trying to withdraw bonus without investing
//     if (!this.bonus?.canWithdraw) {
//       // If they have bonus and haven't activated it
//       const hasBonus = this.bonus?.amount || 0;
//       const regularEarnings = this.wallets.earning - hasBonus;
      
//       // If they're trying to withdraw more than their regular earnings
//       if (amount > regularEarnings) {
//         throw new Error('❌ You must make an investment first to withdraw your bonus! Make an investment to activate your 2,500 FRW bonus.');
//       }
//     }
...
//       youWillReceive: netAmount.toLocaleString()
//     });
...
//     // Create transaction with fee metadata
//     const transaction = await this.createTransactionRecord({
//       type: 'withdraw',
//       amount: withdrawAmount,
//       status: 'pending',
...
//           taxRate: '5%'
//         },
//         bonusWithdrawal: amount <= (this.bonus?.amount || 0) && this.bonus?.canWithdraw,
//         bonusActivated: this.bonus?.canWithdraw || false
//       }
//     });
...
//       reserved: this.wallets.reserved
//     });
//     console.log(`💰 User will receive: ${netAmount.toLocaleString()} FRW after fees`);
...
// // ==================================================
// // ✅ UPDATED: Create investment purchase with bonus activation
// // ==================================================
// userSchema.methods.createInvestment = async function (productId, productName, price, dailyEarning, duration, returnRate) {
//   if (this.wallets.main < price) {
//     throw new Error(`Insufficient balance. Available: ${this.wallets.main.toLocaleString()} FRW`);
...
//   // Add to user's activeInvestments array
//   const userInvestment = {
//     productId: productId,
//     productName: productName,
//     quantity: 1,
...
//   // Add investment ID to investmentIds array
//   if (!this.investmentIds) this.investmentIds = [];
//   this.investmentIds.push(investment._id);
...
//   // ✅ BONUS ACTIVATION: Check if this is first investment to activate bonus
//   const isFirstInvestment = this.activeInvestments.length === 1;
  
//   if (!this.bonus?.investedToActivate && isFirstInvestment) {
//     // Initialize bonus if not exists
//     if (!this.bonus) {
//       this.bonus = {
//         amount: 2500,
...
//     this.bonus.investedToActivate = true;
//     this.bonus.canWithdraw = true;
//     this.bonus.activatedAt = new Date();
    
//     // Add notification about bonus activation
//     await this.addNotification(
//       `🎉 Congratulations! Your bonus of 2,500 FRW is now available for withdrawal! You can withdraw it with 15% service fee + 5% tax applied.`,
//       'success'
//     );
    
//     console.log(`💰 Bonus activated for user ${this.izina_ryogukoresha} after first investment`);
//   }
...
//       dailyEarning: dailyEarning,
//       endDate: endDate,
//       bonusActivated: !this.bonus?.investedToActivate && isFirstInvestment
//     }
//   };
...
//   console.log(`📊 Investment Stats Updated: Total Invested: ${this.investmentStats.totalInvested} FRW, Active: ${this.investmentStats.activeInvestmentCount}`);
//   console.log(`🔗 Investment linked: ${investment._id} added to investmentIds array`);
//   if (isFirstInvestment && !this.bonus?.investedToActivate) {
//     console.log(`💰 BONUS ACTIVATED! User can now withdraw their 2,500 FRW bonus`);
//   }
...
const userSchema = new mongoose.Schema({
  ...
  wallets: {
    main: { type: Number, default: 0 }, // For deposits and investments
    earning: { type: Number, default: 2500 }, // Default bonus + profits
    reserved: { type: Number, default: 0 } // Pending withdrawals
  },
  ...
  bonus: {
    amount: { type: Number, default: 2500 },
    isClaimed: { type: Boolean, default: true }, // Auto-claimed on signup
    canWithdraw: { type: Boolean, default: false }, // Locked until first investment
    investedToActivate: { type: Boolean, default: false },
    createdAt: { type: Date, default: Date.now },
    activatedAt: { type: Date }
  },
...
userSchema.methods.getDashboardData = function () {
  ...
  // Calculate bonus status
  const hasInvestments = this.activeInvestments && this.activeInvestments.length > 0;
  const bonusStatus = {
    amount: this.bonus?.amount || 2500,
    isClaimed: this.bonus?.isClaimed || false,
    canWithdraw: this.bonus?.canWithdraw || false,
    ...
    message: !hasInvestments 
      ? 'Make your first investment to unlock your 2,500 FRW bonus!' 
      : this.bonus?.canWithdraw 
        ? 'Your 2,500 FRW bonus is ready for withdrawal!' 
        : 'Bonus will be available after your first investment matures.'
  };
  ...
  return {
    ...
    igitsina: this.igitsina || '',
    referralCode: this.referralCode,
    referralLink: `/signup?ref=${this.referralCode}`,
    timezone: this.timezone || 'UTC',
    lastPaymentDate: this.lastPaymentDate,
    bonus: bonusStatus,
    wallets: {
...
// Add daily earnings using Investment model
userSchema.methods.addDailyEarnings = async function () {
  const Investment = mongoose.model('Investment');
...
// Check if user needs to be paid today
userSchema.methods.needsPayment = function () {
  if (!this.lastPaymentDate) return true;
...
// Get next payment time in user's timezone
userSchema.methods.getNextPaymentTime = function () {
  const now = new Date();
  const timezone = this.timezone || 'UTC';
...
// ==================================================
// PROFESSIONAL WITHDRAWAL REQUEST WITH BONUS LOGIC
// ==================================================
userSchema.methods.createWithdrawalRequest = async function (amount, paymentMethod, phoneNumber, description) {
  try {
    console.log("🔍 createWithdrawalRequest - START");
    ...
    if (amount < 2500) {
      throw new Error('Minimum withdrawal is 2,500 FRW');
    }
    ...
    // 4. Check if user has made any investments
    const hasInvestments = this.activeInvestments && this.activeInvestments.length > 0;
    
    // 5. Check if user has bonus
    const hasBonus = this.bonus && this.bonus.amount > 0;
    
    // ========== BONUS WITHDRAWAL LOGIC ==========
    
    // If user has bonus but hasn't activated it (no investments)
    if (hasBonus && !this.bonus.canWithdraw && !hasInvestments) {
      throw new Error(
        '❌ Your 2,500 FRW welcome bonus is locked. ' +
        'Make your first investment to unlock it for withdrawal.'
      );
    }
    
    // If user has made investments but bonus is still locked (shouldn't happen normally)
    if (hasInvestments && hasBonus && !this.bonus.canWithdraw) {
      console.warn(`⚠️ User ${this.izina_ryogukoresha} has investments but bonus not activated. Activating now.`);
      this.bonus.canWithdraw = true;
      this.bonus.activatedAt = new Date();
    }
    ...
    // Move money from earning to reserved
    this.wallets.earning = earningBalance - withdrawAmount;
    this.wallets.reserved = (Number(this.wallets.reserved) || 0) + withdrawAmount;
    ...
    // Determine if this withdrawal includes bonus money
    const includesBonus = hasBonus && withdrawAmount <= this.bonus.amount;
    ...
    const transaction = await this.createTransactionRecord({
      ...
      metadata: {
        ...
        bonus: {
          ...
          bonusAmount: includesBonus ? Math.min(withdrawAmount, this.bonus.amount) : 0,
          bonusActivated: this.bonus.canWithdraw
        },
        userStats: {
          hasInvestments: hasInvestments,
          totalInvestments: this.activeInvestments?.length || 0,
          totalEarnings: this.stats?.totalEarned || 0
        }
      }
    });
    ...
    console.log(`💰 User will receive: ${netAmount.toLocaleString()} FRW after fees`);
    console.log(`🎁 Includes bonus: ${includesBonus ? 'Yes' : 'No'}`);
...
// ==================================================
// CREATE INVESTMENT WITH BONUS ACTIVATION
// ==================================================
userSchema.methods.createInvestment = async function (productId, productName, price, dailyEarning, duration, returnRate) {
  try {
    console.log(`💰 Creating investment for ${this.izina_ryogukoresha}: ${productName} - ${price} FRW`);
    ...
    // ========== BONUS ACTIVATION ON FIRST INVESTMENT ==========
    
    let bonusActivated = false;
    
    if (isFirstInvestment && this.bonus && !this.bonus.investedToActivate) {
      // Activate the bonus
      this.bonus.investedToActivate = true;
      this.bonus.canWithdraw = true;
      this.bonus.activatedAt = new Date();
      ...
      // Add notification about bonus activation
      await this.addNotification(
        `🎉 Congratulations! Your 2,500 FRW welcome bonus is now unlocked! You can withdraw it with fees applied.`,
        'success'
      );
    ...
    console.log(`📊 Investment Stats: Total Invested: ${this.investmentStats.totalInvested} FRW, Active: ${this.investmentStats.activeInvestmentCount}`);
    if (bonusActivated) {
      console.log(`🎉 Bonus activated! User can now withdraw their 2,500 FRW bonus`);
    }
