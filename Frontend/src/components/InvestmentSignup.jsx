...
// import { FaSun, FaMoon, FaEye, FaEyeSlash, FaExclamationCircle, FaCheckCircle, FaGift } from 'react-icons/fa';
...
// 💰 Welcome Bonus: 2,500 FRW has been added to your Earning Wallet!
...
//           {/* Bonus Message - Simple and Clean */}
//           <div className="mt-6 pt-4 border-t border-gray-700">
//             <div className={`text-center p-4 rounded-lg ${darkMode ? 'bg-yellow-900/20 border border-yellow-700/30' : 'bg-yellow-50 border border-yellow-200'}`}>
//               <div className="flex items-center justify-center mb-2">
//                 <FaGift className={`mr-2 text-xl ${darkMode ? 'text-yellow-400' : 'text-yellow-600'}`} />
//                 <p className={`font-bold text-xl ${darkMode ? 'text-yellow-300' : 'text-yellow-700'}`}>
//                   🎁 Welcome Bonus: 2,500 FRW
//                 </p>
//               </div>
//               <p className={`text-sm ${darkMode ? 'text-yellow-200' : 'text-yellow-600'}`}>
//                 Get 2,500 FRW instantly in your Earning Wallet when you sign up!
//               </p>
//             </div>
//           </div>
...
const InvestmentSignup = ({ darkMode, setDarkMode }) => {
  ...
  useEffect(() => {
    const checkPhone = async () => {
      if (validation.phoneValid && formData.nimero_yatelefone) {
        try {
          const response = await axios.get(`${API_BASE_URL}/auth/check-phone/${formData.nimero_yatelefone}`);
          setValidation(prev => ({
            ...prev,
            phoneAvailable: response.data.available
          }));
  ...
  useEffect(() => {
    const checkUsername = async () => {
      if (validation.usernameValid && formData.izina_ryogukoresha) {
        try {
          const response = await axios.get(`${API_BASE_URL}/auth/check-username/${formData.izina_ryogukoresha}`);
          setValidation(prev => ({
            ...prev,
            usernameAvailable: response.data.available
          }));
