...
const InvestmentSignup = ({ darkMode, setDarkMode }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ IMPORTANT: Add referral code if it exists
      if (referralCode) {
      ...
      if (response.data.success) {
        ...
        setTimeout(() => {
          ...
          const referralMessage = referralCode 
            ? `\n\n🎁 Watanzwe na: ${referralCode}\nUrakozwe kwinjirwa nawe muri Apex Invest!` 
            : '';
...
👤 Izina: ${user.izina_ryogukoresha}
📱 Nimero: ${user.nimero_yatelefone}
🔗 Kode yo kugana: ${userReferralCode}
🔗 Link yo kugana: ${referralLink}
${referralMessage}
💰 Bonus: 400 RWF yashyizwe mu konti yawe
...
📍 Injira noneho kugirango ukore amasoko!`);
      ...
      if (err.response) {
        switch (err.response.status) {
          case 400:
            setError(err.response.data?.message || 'Amakuru yoherejwe ntabwo ari yo');
            break;
          case 409:
            setError(err.response.data.message || 'Numero ya telefone isanzwe ikoreshwa.');
            break;
        ...
        {referralCode && (
          <div className={`mb-4 p-3 rounded-lg ${darkMode ? 'bg-purple-900/30 border border-purple-700' : 'bg-purple-50 border border-purple-200'}`}>
            <p className={`text-center ${darkMode ? 'text-purple-300' : 'text-purple-700'}`}>
              <span className="font-bold">Utanzwe na: {referralCode}</span>
              <span className="block text-sm mt-1">Urakozwe kwinjirwa nawe!</span>
            </p>
          </div>
        )}
