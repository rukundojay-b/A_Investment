...
//     // Changed to 7 seconds as requested
...
//         // Use products exactly as they come from database
...
//           // Assign icon based on product type/name
...
//     // Check balance only when clicking purchase
//     if (availableBalance < product.price) {
//       alert(`Insufficient balance. You need ${formatCurrency(product.price - availableBalance)} FRW more to purchase this product.`);
//       return;
//     }
...
// 📈 DAILY EARNING: ${formatCurrency(product.dailyEarning)} FRW (${product.returnRate})
// ⏱️ DURATION: ${product.duration}
// 💵 TOTAL RETURN: ${formatCurrency(product.totalReturn)} FRW
// 💹 NET PROFIT: ${formatCurrency(product.totalReturn - product.price)} FRW
// ════════════════════
...
//                     >
//                       {/* Product Image - EXACTLY as your original design */}
...
//                             <span className="text-sm opacity-75">Total Return (25 days):</span>
...
const ProductsCarousel = ({ 
  ...
  const handlePurchaseClick = (product) => {
    ...
    // Professional confirmation modal (using window.confirm as it's standard)
    const confirmMessage = 
`📱 ${product.name}
════════════════════
💰 PRICE: ${formatCurrency(product.price)} FRW
...
Do you want to proceed with this investment?`;
  ...
  const handlePurchase = async (product) => {
    ...
    try {
      await onPurchase(product._id, product.name, product.price);
      showToast('Investment purchased successfully!', 'success');
    } catch (error) {
      console.error('Purchase error:', error);
