      ...
      if (response.data.success && response.data.products && response.data.products.length > 0) {
        const mappedProducts = response.data.products.map((product, index) => {
          ...
          if (type.includes('phone') || type.includes('professional')) {
            ...
            icon = FaGift;
            color = 'yellow';
          } else if (type.includes('pc') || type.includes('business')) {
            icon = FaUsers;
            color = 'purple';
          ...
          if (!imageUrl) {
            ...
            imageUrl = `https://via.placeholder.com/500x300/${color}/ffffff?text=${encodeURIComponent(product.name)}`;
          }
          
          return {
            _id: product._id,
            name: product.name,
            type: product.type || 'Investment',
            price: product.price,
            image: imageUrl,
            dailyEarning: product.dailyEarning || Math.round(product.price * 0.25),
            description: product.description || `Invest in ${product.name} and earn 25% daily returns for 30 days.`,
            duration: product.duration || '30 days',
            returnRate: product.returnRate || '25%',
            icon: icon,
            color: color
          };
            ...
            <div key={slideIndex} className="w-full flex-shrink-0">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                {slideProducts.map((product) => {
                  ...
                  const hasEnoughBalance = availableBalance >= product.price;
                    ...
                    >
                      ...
                      <div className="mb-4">
                        <div className="flex justify-between items-center mb-3">
                          <div>
                            <p className="text-xs opacity-75">Igiciro:</p>
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-lg md:text-xl font-bold">{formatCurrency(product.dailyEarning)} FRW</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        disabled={purchaseLoading[product._id] || !hasEnoughBalance}
                      ...
                      {!hasEnoughBalance && (
                        <p className="text-xs text-red-500 mt-2 text-center">
                          Wukeneye {formatCurrency(product.price - availableBalance)} FRW byongeye
                        </p>
                      )}
                    </div>
                  );
